...
async function buildToolsList() {
    ...
    for (const protocol of cachedProtocols) {
        tools.push({
            ...
            description: protocol.description || `Execute the ${protocol.name || protocol.id} Kitchen Brigade protocol`,
            inputSchema: {
                type: "object",
                properties: {
                    inputs: {
...
async function executeTool(name, args) {
    ...
    if (name === "llm_complete") {
        ...
        if (model_preference === "auto" || model_preference === "local") {
            try {
                ...
                const response = await apiCall("/v1/chat/completions", "POST", {
                    model: localModel,
                    messages,
                    max_tokens,
                    temperature,
                }, INFERENCE_SERVICE_URL, 120000 // Increased timeout for local inference
                );
                return {
                    tier: "local",
    ...
    if (name === "textbook_search") {
        const { query, books, top_k = 5, include_chapters = true, } = args;
        // Route through semantic-search which indexes textbooks
        return apiCall("/v1/search", "POST", {
            query,
    ...
    if (name === "cross_reference_full") {
        ...
        if (sources.includes("qdrant")) {
            searchPromises.push(apiCall("/v1/search", "POST", { query, collection: "all", limit: top_k }, SEMANTIC_SEARCH_URL)
      ...
      `;
        ...
        if (sources.includes("textbooks")) {
            searchPromises.push(apiCall("/v1/search", "POST", { query, collection: "textbooks", limit: top_k }, SEMANTIC_SEARCH_URL)
        ...
        if (sources.includes("code_chunks")) {
            searchPromises.push(apiCall("/v1/search", "POST", { query, collection: "code_chunks", limit: top_k }, SEMANTIC_SEARCH_URL)
