...
async function buildToolsList(): Promise<Tool[]> {
  ...
  const tools: Tool[] = [
    ...
    // =============================================================================
    {
      ...
      description:
        "Audit a repository for code pattern compliance per AUDIT_CODE_PATTERNS_TOOL_SPEC.md. Detects anti-patterns from CODING_PATTERNS_ANALYSIS.md (252 patterns) and SonarQube rules. Set use_semantic=true to use dual-net vector search against code_good_patterns and code_bad_patterns collections for dynamic semantic detection. Returns violations with remediation examples from the pattern library.",
  ...
  for (const protocol of cachedProtocols) {
    if (!protocol?.protocol_id) continue; // Skip protocols without valid protocol_id
    tools.push({
      name: `ai_protocol_${protocol.protocol_id.toLowerCase().replaceAll("-", "_")}`,
      description:
        protocol.description || `Execute the ${protocol.name || protocol.protocol_id} Kitchen Brigade protocol`,
      inputSchema: {
        type: "object",
        properties: {
          inputs: {
...
// =============================================================================
// Test Compliance Audit Tool - Scans repos for test anti-patterns
// Based on TEST_AUDIT_GUIDELINES.md rules AP1-AP4, U2
// =============================================================================
...
interface TestAuditResult {
  ...
  metrics: {
    mock_to_test_ratio: number;
    integration_tests_with_mocks: number;
    tests_without_assertions: number;
  };
  remediation_priority: Array<{ rule: string; count: number; impact: string }>;
  // Cross-reference summary (optional)
  cross_references?: {
    violations_with_examples: number;
    total_examples_found: number;
  };
  // Semantic mode fields (optional)
  semantic_stats?: {
    compliant: number;
    violation: number;
  ...
  manual_review?: Array<{
    ...
    good_score: number;
    bad_score: number;
    top_good_match: string;
    top_bad_match: string;
  }>;
}

// Helper to read directory recursively
async function readDirRecursive(dir: string, pattern: RegExp): Promise<string[]> {
...
async function handleTestComplianceAudit(args: Record<string, unknown>): Promise<TestAuditResult> {
  const {
    repo_path,
    repo_name,
    rules = ["all"],
  ...
  for (const filePath of testFiles) {
    ...
    const testFunctions: Array<{ name: string; startIndex: number }> = [];
  ...
  if (cross_reference && violations.length > 0) {
    ...
    for (const violation of violationsWithSnippets.slice(0, 10)) {
      try {
        const examples = await searchCollection(
          GOOD_PATTERNS_COLLECTION,
  ...
  // Determine grade
  let grade: string;
  if (score >= 95) grade = "EXEMPLARY";
  else if (score >= 85) grade = "COMPLIANT";
  else if (score >= 70) grade = "ACCEPTABLE";
  else if (score >= 50) grade = "NON-COMPLIANT";
  else grade = "CRITICAL";
  
  logTool(`✅ [TEST_AUDIT] Complete: ${repoName} scored ${score}/100 (${grade})`);
  
  const result: TestAuditResult = {
    repo: repoName,
    ...
    remediation_priority: Object.entries(violationsByRule)
      ...
      .sort((a, b) => {
        ...
        return (impactOrder[a.impact as keyof typeof impactOrder] || 3) - (impactOrder[b.impact as keyof typeof impactOrder] || 3);
      }),
  ...
  try {
    ...
    (result as TestAuditResult & { report_path?: string }).report_path = reportFile;
  } catch (err) {
...
interface DualNetResult {
  verdict: "compliant" | "violation" | "manual_review" | "uncertain";
  confidence: number;
  good_matches: SemanticTestMatch[];
  bad_matches: SemanticTestMatch[];
  good_score: number;
  bad_score: number;
  inferred_rules_violated: string[];
  reasoning: string;
}
...
async function searchCollection(
  ...
  try {
    ...
    const qdrantResponse = await apiCall(
      ...
      QDRANT_URL,
      10000
    ) as { result?: Array<{ payload?: Record<string, unknown>; score?: number }> };
    ...
    return qdrantResponse.result.map((r) => ({
      test_id: (r.payload?.test_id as string) || "",
      repo: (r.payload?.repo as string) || "",
      file_path: (r.payload?.file_path as string) || "",
      function_name: (r.payload?.function_name as string) || "",
      compliance_status: (r.payload?.compliance_status as string) || "unknown",
      rules_violated: (r.payload?.rules_violated as string[]) || [],
      patterns_detected: (r.payload?.patterns_detected as string[]) || [],
...
async function checkDualNetCompliance(
  testContent: string,
  testType: string,
): Promise<DualNetResult> {
  ...
  const goodScore = goodMatches.reduce((sum, m, i) => 
    sum + m.similarity * (weights[i] || 0.1), 0);
  const badScore = badMatches.reduce((sum, m, i) => 
    sum + m.similarity * (weights[i] || 0.1), 0);
  ...
  let verdict: DualNetResult["verdict"];
...
// =============================================================================
// Code Pattern Audit Tool - Scans repos for code anti-patterns
// Based on AUDIT_CODE_PATTERNS_TOOL_SPEC.md and CODING_PATTERNS_ANALYSIS.md
// WBS-CDP7: Dual-Net semantic detection using code_good/bad_patterns collections
// =============================================================================

interface SemanticCodeMatch {
  instance_id: string;
  repo: string;
  file: string;
  function_name?: string;
...
interface CodeDualNetResult {
  verdict: "compliant" | "violation" | "manual_review" | "uncertain";
  confidence: number;
  good_matches: SemanticCodeMatch[];
  bad_matches: SemanticCodeMatch[];
  good_score: number;
  bad_score: number;
...
interface CodeViolation {
  ...
  remediation_examples?: RemediationExample[];  // WBS-CDP7: AC-7.4
}
...
interface CodeAuditResult {
  ...
  cross_references?: Array<{
    pattern_id: string;
    similar_fixes: Array<{ repo: string; file: string; diff_hash: string }>;
  }>;
  // WBS-CDP7: AC-7.5 - Semantic mode fields
  semantic_stats?: {
    compliant: number;
    violation: number;
    manual_review: number;
...
async function searchCodePatternCollection(
  collection: string,
  codeContent: string,
  topK: number = 3,
): Promise<SemanticCodeMatch[]> {
  try {
    // Get embedding from semantic-search-service (uses sentence-transformers, not LLM)
    const embedResponse = await apiCall(
      "/v1/embed",
      "POST",
      { text: codeContent.slice(0, 2000) },
      SEMANTIC_SEARCH_URL,
      10000
    ) as { embedding?: number[] };
    ...
    return qdrantResponse.result.map((r) => ({
      instance_id: (r.payload?.instance_id as string) || "",
      repo: (r.payload?.repo as string) || "",
      file: (r.payload?.file as string) || "",
      function_name: (r.payload?.function as string) || undefined,
      rule_id: (r.payload?.rule_id as string) || "",
      code_snippet: (r.payload?.before_code as string) || (r.payload?.after_code as string) || "",
      similarity: r.score || 0,
    }));
  } catch (error) {
...
async function checkCodeDualNet(
  codeContent: string,
  fileContext: string,
): Promise<CodeDualNetResult> {
  // Cast both nets in parallel
  const [goodMatches, badMatches] = await Promise.all([
  ...
  // Dual-net decision logic
  let verdict: CodeDualNetResult["verdict"];
  let reasoning: string;
  let confidence: number;
...
async function handleCodePatternAudit(args: Record<string, unknown>): Promise<CodeAuditResult> {
  const {
    repo_path,
    repo_name,
  ...
  for (const filePath of nonTestFiles) {
    ...
    const functions: Array<{ name: string; startIndex: number }> = [];
    ...
    for (const pattern of CODE_PATTERNS) {
      // Filter by category
      if (!checkCategories.includes(pattern.category)) continue;
      
      // Filter by severity threshold
      if (severityOrder.indexOf(pattern.severity) > thresholdIndex) continue;
      ...
      for (const match of matches) {
        ...
        const alreadyFound = violations.some(
          v => v.file === relativePath && v.line === lineNumber && v.pattern_id === pattern.id
        );
  ...
  let crossReferences: CodeAuditResult["cross_references"];
  let crossRefStats = { violations_with_examples: 0, total_examples_found: 0 };
  ...
  if (cross_reference && violations.length > 0) {
    ...
    for (const violation of violationsNeedingRef.slice(0, 10)) { // Limit to 10
      try {
        const examples = await searchCodePatternCollection(
          CODE_GOOD_PATTERNS_COLLECTION,
          violation.code_snippet || "",
          3
        );
  ...
  try {
    ...
    // Add report path to result
    (result as CodeAuditResult & { report_path?: string }).report_path = reportFile;
  } catch (err) {
...
async function handleAgentFunction(toolName: string, args: Record<string, unknown>): Promise<unknown> {
